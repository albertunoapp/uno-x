	...
	if (sender.url.indexOf('sandbox.html') != -1) {
		...
		if (request.action == 'fetchNextJavascriptRequest') {
			if (pendingJS.length > 0) {
				...
				pendingJS = [];
				console.log('Requesting javascript execution...');
				sendResponse({
		...
		// Retrieve instruction from sandbox
			...
			console.log('Queueing instruction...');
		...
		if (request.action == 'queueJavascriptRequest') {
			console.log('Queueing javascript request...');
...
chrome.app.runtime.onLaunched.addListener(function() {
	chrome.app.window.create('index.html', {
		...
		console.log(launcherWindow);
		launcherWindow.fullscreen(); // Don't "remember" last screen position... just fullscreen it!
