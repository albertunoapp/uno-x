...
var globalLauncherWindow = null;

// Handle communication between launcher and sandbox
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
	if (sender.url.indexOf('sandbox.html') != -1) {
		// If no launcher window found, tell sandbox to close its window
		if (globalLauncherWindow != null) {
			// Launcher window was initialized - check if it was closed
			try {
				// If launcher window is closed, we should not be able to access its properties
				globalLauncherWindow.innerBounds.height;
			} catch(e) {
			...
			// Launcher window was not initialized!
			sendResponse();
			return;
		}
...
chrome.app.runtime.onLaunched.addListener(function() {
	chrome.app.window.create('index.html', {
		...
		globalLauncherWindow = launcherWindow;
